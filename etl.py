...
def process_log_file(cur, filepath):
    ...
    #Filter the dataframe based on the below user Ids for whom we need to set the level to free
    users_to_update = ['73','24','49','16']
    user_update_level_free = user_df
    user_update_level_free = user_update_level_free[user_update_level_free['userId'].isin(users_to_update)].reset_index(drop=True)
    user_update_level_free.head()
    
    #Update user records for the above users
    for i, row in user_update_level_free.iterrows():
        update_userId = (row.userId,)
        cur.execute(user_table_update_free, update_userId)
        
    
    #Filter the dataframe based on the below user Ids for whom we need to set the level to paid
    users_to_update = ['50','54','32','4']
    user_update_level_paid = user_df
    user_update_level_paid = user_update_level_paid[user_update_level_paid['userId'].isin(users_to_update)].reset_index(drop=True)
    user_update_level_paid.head()
    
    #Update user records for the above users
    for i, row in user_update_level_paid.iterrows():
        update_userId = (row.userId,)
        cur.execute(user_table_update_paid, update_userId)
